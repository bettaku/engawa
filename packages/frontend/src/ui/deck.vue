...
<div :class="[$style.root, { [$style.rootIsMobile]: isMobile }]">
		...
		<div ref="columnsEl" :class="[$style.sections, { [$style.center]: prefer.r['deck.columnAlign'].value === 'center', [$style.snapScroll]: snapScroll }]" @contextmenu.self.prevent="onContextmenu" @wheel.self="onWheel">
			...
			<div :class="$style.sideMenu">
				...
				<div :class="$style.sideMenuBottom">
					<button v-vibrate="prefer.s['vibrate.on.system'] ? 5 : []" v-tooltip.noDelay.left="i18n.ts.settings" :class="$style.sideMenuButton" class="_button" @click="showSettings"><i class="ti ti-settings"></i></button>
				</div>
			</div>
		</div>
	</div>
	...
	<div v-if="isMobile" :class="$style.nav">
		...
		<button v-if="store.s.showChatButtonInNavbar" v-vibrate="prefer.s['vibrate.on.system'] ? 5 : []" :class="$style.navButton" class="_button" @click="mainRouter.currentRoute.value.name === 'chat' ? top() : mainRouter.push('/chat')"><i :class="$style.navButtonIcon" class="ti ti-messages"></i><span v-if="$i != null && $i.hasUnreadChatMessages" :class="$style.navButtonIndicator" class="_blink"><i class="_indicatorCircle"></i></span></button>
