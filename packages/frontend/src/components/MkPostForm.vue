...
>
	<header :class="$style.header">
		...
		<div :class="$style.headerRight">
			...
			<button v-click-anime v-tooltip="i18n.ts._visibility.disableFederation" class="_button" :class="[$style.headerRightItem, { [$style.danger]: localOnly }]" :disabled="targetChannel != null || visibility === 'private'" @click="toggleLocalOnly">
				<span v-if="!localOnly"><i class="ti ti-rocket"></i></span>
				<span v-else><i class="ti ti-rocket-off"></i></span>
			</button>
			<button ref="otherSettingsButton" v-tooltip="i18n.ts.other" class="_button" :class="$style.headerRightItem" @click="showOtherSettings"><i class="ti ti-dots"></i></button>
			<div :class="$style.submit">
				<button v-click-anime class="_button" :class="$style.submitButton" :disabled="!canPost" data-e2e-open-post-form-submit @click="post">
	...
	<div :class="[$style.textOuter, { [$style.withCw]: useCw }]">
		...
		<textarea ref="textareaEl" v-model="text" :class="[$style.text]" :disabled="posting || posted" :readonly="textAreaReadOnly" :placeholder="placeholder" data-e2e-post-form-text @keydown="onKeydown" @keyup="onKeyup" @paste="onPaste" @compositionupdate="onCompositionUpdate" @compositionend="onCompositionEnd"/>
...
if (replyTargetNote.value && ['home', 'followers', 'specified', 'private'].includes(replyTargetNote.value.visibility)) {
...
function watchForDraft() {
	...
	watch(scheduledNoteDelete, () => saveDraft());
	watch(searchableBy, () => saveDraft(), { deep: true });
}
...
function clear() {
	...
	searchableBy.value = store.s.searchableBy;
}
	...
	nextTick(() => {
		...
		if (!props.instant && !props.mention && !props.specified && !props.mock) {
			...
			if (draft) {
				...
				searchableBy.value = draft.data.searchableBy ?? store.s.searchableBy;
			}
		}
		...
		if (props.initialNote) {
			...
			searchableBy.value = init.searchableBy ?? store.s.searchableBy;
		}
